...
class SqliteDBExecutor(BaseSQLDBExecutor):
    ...
    def _install_timeout_listener(self, max_ms=1_000, vm_steps=1_000):
        ...
        self.logger.warning(f'Set execution timeout to {max_ms / 1000} seconds. If you want to change it, call `set_different_timeout()` or initialize class with different timeout.')
    ...
    def execute_multiple_query(self, queries: list[str | sql.Executable],
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_thread) as executor:
            futures = {
                executor.submit(self.execute_query, q, p, throw_if_error=throw_if_error): i
