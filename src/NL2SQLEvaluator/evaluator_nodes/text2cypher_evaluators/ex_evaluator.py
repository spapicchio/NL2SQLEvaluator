from NL2SQLEvaluator.evaluator_nodes.evaluator_input import EvalText2CypherTask
...
@register_node(package_name='text2cypher_evaluators')
class CypherEXEvaluator:
    """Execution accuracy evaluator for Text2Cypher tasks.

    Delegates comparison to ``CypherExecutorOutput.is_equivalent_to``,
    which handles column-permutation backtracking automatically.
    """

    def execute_metric(
            self,
            tasks: list[EvalText2CypherTask],
            *args,
            **kwargs
    ) -> list[float]:
    ...
    def _ex(self, task: EvalText2CypherTask, is_row_order_important: bool) -> float:
