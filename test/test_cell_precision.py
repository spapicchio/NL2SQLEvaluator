...
from NL2SQLEvaluator.metric_executor.orchestrator import orchestrator_worker, OrchestratorInput
...
class TestCellPrecision:
    ...
    def orchestrator_input(self):
        return {
            "executor": None,
            "metrics_to_calculate": ["cell_precision"]
        }
    ...
    def test_equal(self, orchestrator_input):
        ...
        result = orchestrator_worker.invoke(orchestrator_input)
        assert result[0]['cell_precision'] == 1.0
    ...
    def test_empty_vs_non_empty(self, orchestrator_input):
        ...
        orchestrator_input = OrchestratorInput(**orchestrator_input)
        result = orchestrator_worker.invoke(orchestrator_input)
        assert result[0]['cell_precision'] == 0.0
    ...
    def test_float_outside_epsilon(self, orchestrator_input):
        ...
        orchestrator_input = OrchestratorInput(**orchestrator_input)
        result = orchestrator_worker.invoke(orchestrator_input)
        assert result[0]['cell_precision'] == 0.0  # assuming default epsilon=1e-6
    ...
    def test_partial_matching_cells(self, orchestrator_input):
        orchestrator_input["target_queries"] = [[(math.nan, 'b'), ('c', 'd')]]
        orchestrator_input["predicted_queries"] = [[(math.nan, 'x'), ('y', 'd')]]
        orchestrator_input = OrchestratorInput(**orchestrator_input)
        result = orchestrator_worker.invoke(orchestrator_input)
        assert result[0]['cell_precision'] == 0.5

    def test_single_cell_table(self, orchestrator_input):
        orchestrator_input["target_queries"] = [[('1.0', None)]]
        orchestrator_input["predicted_queries"] = [[(None,), ('1.0',)]]
        orchestrator_input = OrchestratorInput(**orchestrator_input)
        result = orchestrator_worker.invoke(orchestrator_input)
        assert result[0]['cell_precision'] == 1.0
    ...
    def test_special_chatgpt_case(self, orchestrator_input):
        orchestrator_input["target_queries"] = [[('573585', '10/31/2019', '22282', '12-Egg-House-Painted-Wood',
